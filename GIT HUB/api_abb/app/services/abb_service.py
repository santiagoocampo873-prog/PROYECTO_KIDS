...
from app.models.abb_model import BinarySearchTree, Child
...
class ABBService:
    """
    Servicio que encapsula la lógica de negocio del Árbol Binario de Búsqueda.
    Actúa como intermediario entre el controlador y el modelo del ABB.
    Sigue el principio de separación de responsabilidades (Single Responsibility Principle).
    """
    
    def __init__(self):
        """
        Constructor del servicio ABB.
        Inicializa una instancia única del árbol binario de búsqueda.
        """
    ...
    def add_child(self, child_data: ChildCreate) -> dict:
        ...
        if success:
            return {
                "success": True,
                "message": f"Niño '{child.name}' con ID {child.id} agregado exitosamente al árbol",
    ...
    def get_tree_structure(self) -> TreeResponse:
        ...
        Útil para visualizar el árbol de forma jerárquica.
